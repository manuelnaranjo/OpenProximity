...
class ModelNode(object):
	...
	for related in obj._meta.get_all_related_objects():
		...
		for subk in get_subclasses(related.model):
			...
			self.node['children'].append( ModelNode(qs.get()).getDict() )
    def getDict(self):
	return self.node
...
class RelatedNode(object):
	...
	for curr in qs[::step]:
	    if prev:
		url = generic_URL(app, model, related_field, related_pk, prev, curr.pk)
...
def relatedDetail(request, app, model, rel_field, rel_pk):
    ...
    return HttpResponse( 
	simplejson.dumps(RelatedNode(qs, app, model, rel_field, rel_pk).getDict()), 
	content_type="application/json")
