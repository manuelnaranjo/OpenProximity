...
class InteligentResponder(XMLResponder, JSONResponder):
    # types of responder this list is:
    # [html_accept type] = (responder_to_use, serializer_format, mime_type)
    types = {
             'xml':  (XMLResponder, 'xml', 'application/xml'),
             'json': (JSONResponder, 'json', 'application/json'),
    }
    def __init__(self, paginate_by=None, allow_empty=False):
        SerializeResponder.__init__(self,"", "", paginate_by=paginate_by, allow_empty=allow_empty)

    def __getAcceptTypes(self, request):
        def internal_accept(types):
            for t in types:
                if '/' in t: t = t.split('/', 1)[1]
                if ';' in t: t = t.split(';', 1)[0]
                if '+' in t: 
                    for a in t.split('+'): yield a
                else: yield t
        if not "HTTP_ACCEPT" in request.META:
            raise Exception("No HTTP Accept Header")
        types = request.META["HTTP_ACCEPT"].split(",")
        for allow in internal_accept(types):
            if allow in self.types:
                self.format, self.mimetype = self.types[allow][1:]
                return self.types[allow][0]
        self.format, self.mimetype = self.types['json'][1:]
        return self.types['json'][0]

    def __do_call(self, method, request, *args, **kwargs):
        resp = self.__getAcceptTypes(request)
        method = getattr(resp, method)
        return method(self, request, *args, **kwargs)

    def error(self, *args, **kwargs):
        return self.__do_call('error', *args, **kwargs)
    
    def list(self, *args, **kwargs):
        return self.__do_call('list', *args, **kwargs)

    def element(self, *args, **kwargs):
        return self.__do_call('element', *args, **kwargs)
