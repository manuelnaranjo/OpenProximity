...
class InteligentReceiver(Receiver):
    ...
    default = FormReceiver()

    def __init__(self):
        super(InteligentReceiver, self)

    def __getAcceptTypes(self, request):
        if not "CONTENT_TYPE" in request.META:
            return self.default
        type = request.META["CONTENT_TYPE"].split("/")[1]
        return self.types.get(type, self.default)
    ...
    def get_data(self, request, method):
        return self.__getAcceptTypes(request).get_data(request, method)
